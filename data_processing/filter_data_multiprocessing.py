...
# process dataset
def process_dataset(hf: h5.File, particle: str, part: str, ds: str, mask_evs: np.ndarray, mask_hits: np.ndarray, num_un_strings: np.ndarray, ev_starts: np.ndarray) -> Tuple[np.ndarray, str, Dict]:
    kwargs = {}
    ...
    return n_data, rs, kwargs

# process part
def process_part(args_q: Queue, result_q: Queue, do_quit: Event):
    # with try to catch possible error
    try:
        # loop that can be broekn via external event
        while not do_quit.is_set():
            args = args_q.get()
            if args is None:
                break
            else:
                hf_path, particle, part = args
                with h5.File(hf_path, 'r') as hf:
                    ...
                    for ds in config['keys_to_pull']:
                        data, path, kwargs = process_dataset(hf, particle, part, ds, mask_evs, mask_hits, num_un_strings, ev_starts)
                        result_q.put( (particle, path, part, data, kwargs) )
...
def listener(result_q: Queue, do_quit: Event, n_workers: int):
    err = None
    with h5.File(config['h5_out'],'w') as hf:
        while n_workers > 0:
            res = result_q.get()
            if res is None:
                n_workers -= 1
                ...
                try:
                    hf.create_dataset(f"{res[0]}/{res[1]}/{res[2]}/data", data=res[3], **res[4])
                except Exception as err_:
                    do_quit.set() # let main continue
                    err = err_ # do not rise error here as queue will hang
    # now, when all proc finished, rise error
    if err is not None:
        raise err
    # we are done
    do_quit.set() # let main continue
...
def main() -> None:
    ...
    # init multiprocessing things
    args_q = Queue()
    result_q = Queue(maxsize=MAX_QUEUE_SIZE)
    do_quit = Event()
    
    listener_p = Process(target=listener, args=(result_q, do_quit, NUM_WORKERS))
    listener_p.start()

    for _ in range(NUM_WORKERS):
        worker_p = Process(target=process_part, args=(args_q, result_q, do_quit))
        worker_p.start()
    logging.info(f"Initiated {NUM_WORKERS} workers with {MAX_QUEUE_SIZE} maximal queue size")

    with h5.File(config['h5_in'], 'r') as hf:
        for particle in config['particles']:
            ...
            # multiprocessing
            for part in parts:
                args_q.put( (config['h5_in'], particle, part) )
        
        # ensure that processes will terminate
        for _ in range(NUM_WORKERS):
            args_q.put(None)
