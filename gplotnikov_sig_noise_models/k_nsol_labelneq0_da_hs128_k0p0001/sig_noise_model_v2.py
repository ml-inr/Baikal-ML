...
Used by predict_*.py scripts.

v2: GPU-chunked inference with torch.nn.utils.rnn.pad_sequence.

This variant wraps EncoderDomainAdaptation (encoder.py) for inference:
the domain-adaptation head is discarded; only per-hit classification
logits are returned.
"""
...
# ── Dynamic import of encoder.py (has relative imports) ────────────────

def _load_encoder_module() -> types.ModuleType:
    """Load encoder.py from MODEL_DIR, resolving its relative import of layers."""
    pkg = "_snm_pkg"

    # Create a synthetic package so relative imports in encoder.py resolve
    if pkg not in sys.modules:
        pkg_mod = types.ModuleType(pkg)
        pkg_mod.__path__ = [str(MODEL_DIR)]
        pkg_mod.__package__ = pkg
        sys.modules[pkg] = pkg_mod

    # Load layers.py as <pkg>.layers
    layers_key = f"{pkg}.layers"
    if layers_key not in sys.modules:
        spec = importlib.util.spec_from_file_location(layers_key, MODEL_DIR / "layers.py")
        layers_mod = importlib.util.module_from_spec(spec)
        layers_mod.__package__ = pkg
        sys.modules[layers_key] = layers_mod
        spec.loader.exec_module(layers_mod)

    # Load encoder.py as <pkg>.encoder
    encoder_key = f"{pkg}.encoder"
    if encoder_key not in sys.modules:
        spec = importlib.util.spec_from_file_location(encoder_key, MODEL_DIR / "encoder.py")
        encoder_mod = importlib.util.module_from_spec(spec)
        encoder_mod.__package__ = pkg
        sys.modules[encoder_key] = encoder_mod
        spec.loader.exec_module(encoder_mod)

    return sys.modules[encoder_key]


_encoder_mod = _load_encoder_module()
EncoderDomainAdaptation = _encoder_mod.EncoderDomainAdaptation


# ── Inference wrapper ───────────────────────────────────────────────────

class _InferenceModel(nn.Module):
    """Strips the domain-adaptation head; returns only classification output."""

    def __init__(self, da_model: EncoderDomainAdaptation) -> None:
        super().__init__()
        self.model = da_model

    def forward(self, x: torch.Tensor, mask: torch.Tensor) -> torch.Tensor:
        output, _ = self.model(x, mask)
        return output  # (B, L, out_size) when aggregate_output=False
...
def load_model(
    ...
    """Load EncoderDomainAdaptation from checkpoint, wrapped for inference.

    Returns:
        (model, config, device)  where model returns only classification logits.
    """
