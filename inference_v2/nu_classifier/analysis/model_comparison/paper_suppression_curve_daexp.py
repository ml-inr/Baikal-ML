...
Outputs tables/paper_suppression_curve_daexp.csv + figures/paper_suppression_curve_daexp.png.
"""
...
FT_BG_FKS_PATH = ROOT/'inference_v2/nu_classifier/exp_finetuning/exp_bg_datasets/260705_0702_da_nu_classifier_exp_full_E1_lambda0.01@da_checkpoint_epoch_010_ood2p5_rv0p7/exp_bg_event_fks.npy'
...
# Exclude only LABEL-LEAKED events: MC training (labelled 0/1) and the FT background
# (labelled 0). The DA target exp was UNLABELLED (domain alignment only, no class leak), so
# those exp events are kept — they are valid for scoring, and give ~3× more exp statistics.
MC_TRAIN = train_keys(MC_TRAIN_NPY, 'h5_part_keys.npy', 'h5_local_event_ids.npy')
FT_BG_FKS = set(np.load(FT_BG_FKS_PATH).astype(np.int64).tolist())
print(f"  MC train keys={len(MC_TRAIN):,}  FT-background event_fks={len(FT_BG_FKS):,}", flush=True)
...
def load(ckpt_dir):
    ...
    d=c.execute(f"""SELECT pr.score, pr.event_fk FROM e.predictions pr
      JOIN cat.events ev ON ev.id=pr.event_fk
      WHERE {CUT} AND NOT(ev.cluster=2 AND ev.run IN('20','249'))""").df()
    ex=d.score.to_numpy()[~d.event_fk.isin(FT_BG_FKS).to_numpy()]   # exclude only FT-labelled bg
...
for mu in MU_GRID:
    ...
    rows.append(r)
tab=pd.DataFrame(rows); tab.to_csv(HERE/'tables/paper_suppression_curve_daexp.csv',index=False)
pd.set_option('display.width',240,'display.max_columns',40)
print('\n=== E1 vs Fine-tuned — out-of-training, muon suppression to 1e-6 (DA-exp kept, FT-bg excluded) ===')
...
fig.tight_layout();fig.savefig(HERE/'figures/paper_suppression_curve_daexp.png',dpi=140)
print('saved figures/paper_suppression_curve_daexp.png')
