#!/usr/bin/env python
"""Working-point SWEEP: does the horizon θ-loss (E5, converged @ep8) reduce the exp
...
unaffected. Outputs tables/working_point_sweep.csv + figures/working_point_sweep.png.
"""
...
E5_CKPT = ROOT/'experiments/numu/260707_0226_da_nu_classifier_exp_full_E5_horizon_lambda0.01/best_da_model.pth'  # ep8
...
print('scoring E5 …',flush=True); e5=score_all(E5_CKPT)

rows=[]
for mu_surv in MU_GRID:
    r={'mu_surv':mu_surv}
    for tag,(s_mu,s_nu,s_ex) in [('E1',e1),('E5',e5)]:
        cut=np.quantile(s_mu,1.0-mu_surv)
        r[f'{tag}_cut']=cut
        r[f'{tag}_sig_eff']=float((s_nu>cut).mean())
        r[f'{tag}_exp_frac']=float((s_ex>cut).mean())
    r['exp_ratio_E5_over_E1']=r['E5_exp_frac']/r['E1_exp_frac'] if r['E1_exp_frac']>0 else np.nan
    rows.append(r)
tab=pd.DataFrame(rows); tab.to_csv(HERE/'tables/working_point_sweep.csv',index=False)
pd.set_option('display.width',200,'display.max_columns',20)
print('\n=== E1 (ep10) vs E5 horizon (ep8) at matched muon working points ===')
...
ax[0].plot(tab.mu_surv,tab.E5_exp_frac,'s-',label='E5 horizon (ep8)',color='tab:red')
...
fig.tight_layout(); fig.savefig(HERE/'figures/working_point_sweep.png',dpi=130)
print('saved figures/working_point_sweep.png')
