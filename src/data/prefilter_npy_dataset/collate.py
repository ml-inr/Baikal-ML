"""Collate function for PrefilterNpyDataset batches.
...
def prefilter_collate_fn(
    ...
    Args:
        batch: List of dicts from ``PrefilterNpyDataset.__getitem__``.
    ...
    # Mask: True = real hit, False = padding
    ...
    if augmentation_config is not None:
        ...
        if noise_std_list is not None:
            noise_std = torch.tensor(noise_std_list, dtype=torch.float32, device=device)
            noise = torch.randn_like(padded) * noise_std
            padded = torch.where(mask.unsqueeze(-1), padded + noise, padded)

            # Re-sort by time (feature index 1), keeping padding at the tail.
            # Set padding positions to +inf so they always sort last.
    ...
    return {
        "features": padded,                   # (B, max_len, 5)
        "labels": labels,                     # (B,) float32
        "lengths": lengths,                   # (B,)
        "original_lengths": original_lengths,  # (B,)
        "mask": mask,                         # (B, max_len) bool
        "signal_hit_counts": signal_hit_counts,    # (B,)
        "signal_string_counts": signal_string_counts,  # (B,)
        "particle_types": particle_types,     # (B,)
    }
