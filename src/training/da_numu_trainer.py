...
class DomainAdaptationTrainer:
    ...
    - Classification head for neutrino/muon prediction (MC only)
    - Domain discriminator for MC/Exp prediction (muon events only)
    - Adversarial training with gradient reversal
    
    CRITICAL DESIGN: Uses only muon events for domain discrimination to avoid
    the feature extractor learning that "MC domain contains neutrinos while 
    Exp domain contains only muons", which would confound neutrino detection.
    ...
    def prepare_data(self):
        ...
        self.source_train_loader = create_numu_dataloader_from_ds(
        ...
        self.source_val_loader = create_numu_dataloader_from_ds(
        ...
        self.target_train_loader = create_numu_dataloader_from_ds(
        ...
        self.target_val_loader = create_numu_dataloader_from_ds(
    ...
    def train_epoch(self) -> Dict[str, float]:
        ...
        for batch_idx in range(total_batches):
            ...
            # Filter source batch to keep only muon events (label=0)
            source_muon_mask = (source_labels == 0).squeeze()
            n_source_muons = source_muon_mask.sum().item()
            n_target_events = target_batch['features'].shape[0]
            
            # Skip domain training if no muons in source batch (very rare with large batches)
            if n_source_muons == 0:
                logger.warning("No muons in source batch - skipping domain training step")
            ...
            # MINIMUM BATCH STRATEGY: Use min(source_muons, target_events) from each domain
            domain_batch_size = min(n_source_muons, n_target_events)
            
            # Sample domain_batch_size events from each domain
            # Source domain: randomly select from available muons
            source_muon_indices = torch.where(source_muon_mask)[0]
            if n_source_muons > domain_batch_size:
                selected_source_indices = source_muon_indices[torch.randperm(n_source_muons, device=self.device)[:domain_batch_size]]
            else:
                selected_source_indices = source_muon_indices
            
            # Target domain: randomly select from all events (all muons)
            ...
            # Create domain labels (0=source/MC muons, 1=target/Exp muons)
            ...
            source_utilization = domain_batch_size / n_source_muons
            ...
            with torch.no_grad():
                source_domain_preds = torch.sigmoid(source_domain_logits) < 0.5  # Should be 0 (MC muons)
                target_domain_preds = torch.sigmoid(target_domain_logits) >= 0.5  # Should be 1 (Exp muons)
            ...
            epoch_metrics['source_muons_avg'] += n_source_muons
            ...
            if batch_idx % log_every == 0:
                ...
                logger.info(f"  Domain Batches: {domain_batch_size} (from {n_source_muons} source muons, "
        ...
        metrics_to_average = ['classification_loss', 'domain_loss', 'total_loss', 'domain_accuracy', 
                             'domain_utilization', 'source_muons_avg', 'target_events_avg', 'domain_batch_size_avg']
    ...
    def validate_epoch(self) -> Dict[str, float]:
        ...
        with torch.no_grad():
            ...
            for _ in range(val_batches):
                ...
                # Skip if no muons in source validation batch
                if n_source_muons == 0:
                    continue
                ...
                source_muon_indices = torch.where(source_muon_mask)[0]
                if n_source_muons > domain_batch_size:
                    selected_source_indices = source_muon_indices[:domain_batch_size]
                else:
                    selected_source_indices = source_muon_indices
