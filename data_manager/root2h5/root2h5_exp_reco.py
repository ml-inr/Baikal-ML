"""
Convert experimental ROOT files with reconstruction to HDF5.

Based on root2h5_exp.py. Adds:
- reco_prty: per-event reconstruction parameters (analogous to prime_prty in MC)
- raw/labels: per-hit RecoMuonMask flags (analogous to MC hit labels)

Reco filename format: 2020_cl{C}_run{R}_scl_nu_DATA2020.root
Event ID format:      exp_s2020_c{CC}_r{RRRR}_{N}  (matching exp.h5 convention)

Usage:
    cd data_manager/root2h5
    python root2h5_exp_reco.py
"""
...
try:
    config = read_config('root2h5_config_exp_reco.yaml')
...
pathes_header = config['root_paths'].get('header', [])
pathes_reco_ev = config['root_paths']['reco_ev']
pathes_reco_hit = config['root_paths'].get('reco_hit', [])
path_reco_mask = config['root_paths']['reco_mask']

MAX_QUEUE_SIZE = config['multiprocessing']['MAX_QUEUE_SIZE']
NUM_WORKERS = config['multiprocessing']['NUM_WORKERS']


# --- Filename → ID prefix conversion ---

def reco_filename_to_id_prefix(filename: str) -> str:
    """Convert reco filename to exp.h5-compatible ID prefix.

    '2020_cl1_run27_scl_nu_DATA2020' → 's2020_c01_r0027'
    """
    m = re.match(r'(\d{4})_cl(\d+)_run(\d+)', filename)
    if not m:
        raise ValueError(f"Cannot parse reco filename: {filename}")
    year, cluster, run = m.group(1), int(m.group(2)), int(m.group(3))
    return f"s{year}_c{cluster:02d}_r{run:04d}"


# --- Helper functions (same as root2h5_exp.py) ---
...
def get_start_index(rf):
    pulses_nums = rf['Events/BEvent./BEvent.fPulseN'].array(library="np")
    mask = pulses_nums > 0
    if not np.any(mask):
        return len(pulses_nums)
...
def get_dtype(key, value):
    if any(s in key for s in ['data', 't_res', 'reco_prty']) or ('muons' in key and 'starts' not in key):
        return np.float32
    elif 'ev_ids' in key or 'header_prty' in key:
        return value.dtype   # preserve int32/uint32 as-is
    else:
        return np.int32
...
def write_data(hf, res, file_num):
    for key, value in res.items():
        ...
        kwarg = {
            ...
            'compression': 'gzip' if any(s in key for s in ['raw', 'reco']) and 'starts' not in key else None
        }
    ...
    while not do_quit.is_set():
        ...
        try:
            ...
            with ur.open(rf_path) as rf:
                ...
                # --- Read header scalars (per-event: season, cluster, run, eventIDCC) ---
                if pathes_header:
                    header_matrix = np.column_stack(
                        [rf[p].array(library="np")[st:] for p in pathes_header]
                    )  # (n_events, 4)

                # --- Read reco scalars (per-event, before cluster splitting) ---
                reco_ev_arrays = []
                for rp in pathes_reco_ev:
                    arr = rf[rp].array(library="np")[st:]
                    reco_ev_arrays.append(arr)
                reco_ev_matrix = np.column_stack(reco_ev_arrays)  # (n_events, n_reco_fields)

                # --- Read reco mask (per-hit, variable-length per event) ---
                reco_mask_raw = rf[path_reco_mask].array(library='np')[st:]
                ...
                # --- Read per-hit reco fields (same jagged structure as hit data) ---
                reco_hit_data = [rf[p].array(library="np")[st:] for p in pathes_reco_hit]

                # --- Split multi-cluster events (hit data + reco mask + reco hit) ---
                if split_multi:
                    ...
                    # Split reco mask the same way as hit data
                    reco_mask_multi = flatten_multi(
                        [reco_mask_raw[mask_multi]],
                        channels[mask_multi],
                        cluster_ids_multi
                    )[0]
                    reco_mask_multi = np.array(reco_mask_multi, dtype=object)

                    # Split per-hit reco fields the same way
                    if reco_hit_data:
                        reco_hit_multi = flatten_multi(
                            [d[mask_multi] for d in reco_hit_data],
                            channels[mask_multi],
                            cluster_ids_multi
                        )
                        reco_hit_multi = [np.array(d, dtype=object) for d in reco_hit_multi]
                ...
                if take_single_cluster:
                    ...
                    if reco_hit_data:
                        reco_hit_single = [d[mask_single] for d in reco_hit_data]
                    ...
                    if split_multi:
                        ...
                        if reco_hit_data:
                            reco_hit_single = [
                                np.concatenate((s, m), axis=0)
                                for s, m in zip(reco_hit_single, reco_hit_multi)
                            ]

                # --- Apply cluster splitting to per-event scalars ---
                reco_prty = cast_to_single(
                    reco_ev_matrix, mask_single, mask_multi,
                    nums_multi, take_single_cluster, split_multi
                )
                if pathes_header:
                    header_prty = cast_to_single(
                        header_matrix, mask_single, mask_multi,
                        nums_multi, take_single_cluster, split_multi
                    )
                ...
                reco_mask_flat = np.concatenate(reco_mask_events)
                ...
                reco_mask_flat = reco_mask_flat[sort_idxs]

                # Flatten and sort per-hit reco fields the same way
                if reco_hit_data:
                    # fTRes may cover fewer hits than pulse count — pad with NaN to align
                    for fi in range(len(reco_hit_single)):
                        aligned = []
                        for arr, n in zip(reco_hit_single[fi], num_channels):
                            arr = arr.astype(np.float32)
                            n = int(n)
                            if len(arr) < n:
                                pad = np.full(n - len(arr), np.nan, dtype=np.float32)
                                aligned.append(np.concatenate([arr, pad]))
                            else:
                                aligned.append(arr[:n])
                        reco_hit_single[fi] = np.array(aligned, dtype=object)
                    reco_hit_flat = np.array([np.concatenate(d) for d in reco_hit_single])
                    reco_hit_flat = reco_hit_flat[:, sort_idxs]
                ...
                if exclude_big_ts:
                    ...
                    if reco_hit_data:
                        reco_hit_flat = reco_hit_flat[:, mask]
                    shifts = np.array([
                ...
                res_dict['raw/labels'] = reco_mask_flat.astype(np.int32)
                ...
                # Header per-event properties (season, cluster, run, eventIDCC)
                if pathes_header:
                    res_dict['header_prty'] = header_prty

                # Reco per-hit properties (n_hits × n_fields), if configured
                if reco_hit_data:
                    res_dict['reco_hit_prty'] = np.transpose(reco_hit_flat).astype(np.float32)
...
def main():
    ...
    logging.info("Reco data processing started")
    ...
    filenames = []
    cl_centers = []
    root_files = [f for f in os.listdir(root_dir_path) if f.endswith('.root')]
    ...
    for i, root_file in enumerate(root_files):
        ...
        # Convert reco filename to exp-compatible ID prefix
        raw_name = root_file.split(".")[0]
        id_prefix = reco_filename_to_id_prefix(raw_name)

        args_q.put((rf_path, id_prefix, cl_center))
        filenames.append(id_prefix)
        cl_centers.append(cl_center)
    cl_centers = np.asarray(cl_centers)
