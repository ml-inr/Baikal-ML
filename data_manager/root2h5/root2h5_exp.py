...
try:
    config = read_config('root2h5_config_exp.yaml')
...
def get_start_index(rf):
    ...
    mask = pulses_nums > 0
    if not np.any(mask):
        return len(pulses_nums) # No valid events
        ...
        try:
            ...
            with ur.open(rf_path) as rf:
                ...
                if shift_coords_to_cl_center:
                    logging.info(f"Shifting coordinates to cluster centers.")
                    logging.info(f"\t{coordinates.shape=}")
                    logging.info(f"\t{cl_centers.shape=}")
                    cl_centers_broad = np.expand_dims( np.repeat( np.transpose(cl_centers, axes=(1,0)), 288, axis=1), axis=1 )
                    logging.info(f"\t{cl_centers_broad.shape=}")
                    coordinates += -cl_centers_broad
                    logging.info(f"\tMean Corrds values after shifting: {coordinates.mean(axis=(1,2))=}")
                    # recalculate coordinates
                    channels = r_data[2].astype('int')
                    try:
                        tr_coord = [ [coordinates[j,i,channels[ev_starts[i]:ev_starts[i+1]]] for i in range(len(ev_starts)-1)] for j in range(3) ]
...
if __name__ == "__main__":
    import os
    if not os.path.exists(os.path.join(h5_prefix, h5_name)):
        with h5.File(os.path.join(h5_prefix, h5_name), 'w') as hf:
            pass
    main()
