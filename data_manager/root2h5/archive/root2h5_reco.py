'''
To do: write normal code for getting reco parameters
'''
...
from eval_tres_reco import eval_tres
...
try:
    config = read_config('root2h5_config_reco.yaml')
...
input_dir_path = config['input']['dir_path']
...
pathe_reco_mask = config['root_paths']['reco_mask']
#pathes_reco_om = config['root_paths']['reco_om']
pathes_reco_vector = config['root_paths']['reco_vectors']
    ...
    while not do_quit.is_set():
        ...
        try:
            ...
            with ur.open(rf_path) as rf:
                ...
                ### RECO
                data = [rf[p].array(library="np")[st:] for p in pathes_data + [pathe_reco_mask]]
                ###
                ...
                channels = r_data[3].astype(np.int32)
                ...
                ### RECO
                #res_dict['reco_om/data'] = np.transpose( r_data[[4:-3]], (1,0) )
                reco_ev = np.array([ rf[p].array(library="np")[st:] for p in pathes_reco_ev ])
                res_dict['reco_ev'] = cast_to_single(np.transpose( reco_ev, (1,0) ), 
                                                 mask_single, mask_multi, nums_multi, take_single_cluster, split_multi)
                reco_mask = np.array(rf[pathe_reco_mask].array(library="np")[st:])
                #print('-----')
                #print(reco_mask.shape)
                #print(reco_mask[0].shape)
                #res_dict['reco_mask'] = np.concatenate(cast_to_single(reco_mask, 
                #                                 mask_single, mask_multi, nums_multi, take_single_cluster, split_multi))              
                res_dict['reco_mask'] = r_data[4]
                #print(res_dict['reco_mask'].shape)
                reco_track = np.array(ak.unzip([ rf[p].array()[1:] for p in pathes_reco_vector ]))
                res_dict['reco_track'] = cast_to_single(np.transpose( reco_track, (2,1,0) ), 
                                                 mask_single, mask_multi, nums_multi, take_single_cluster, split_multi)
...
def main():
    ...
    cl_centers = None
    root_files = [f for f in os.listdir(input_dir_path) if f.endswith('.root')]

    for root_file in root_files:
        rf_path = os.path.join(input_dir_path, root_file)
