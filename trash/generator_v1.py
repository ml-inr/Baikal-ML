...
try:
    ...
    from data.config import GeneratorConfig, ChunkGeneratorConfig
except ImportError:
    from root_manager.processor import Processor
    from settings import GeneratorConfig, ChunkGeneratorConfig
...
class BatchGenerator:
    ...
    def __init__(self, chunk_generator: ChunkGenerator, collect_stats=False):
        self.cfg = cfg
    ...
    def _to_padded_batch(self, df: pl.DataFrame) -> tuple[list[list[list]]]:
        ...
        # Dynamically update name-index correspondance
        ...
        for chunk in chunks:
            ...
            for start in range(0, chunk.shape[0], bs):
                ...
                if pre_batch.shape[0] == 0:
                    ...
                    break
                data = self._to_padded_batch(pre_batch[self.cfg.features])
                data = torch.tensor(data).transpose(1,2)
                logging.debug(f"Coverting data to tensor of shape: {data.shape}")
                
                if self.cfg.do_augment:
                    if self.aug_stds is None:
                        self.aug_stds = [0.] * data.shape[2]
                        for name, std in self.aug_params.to_dict().items():
                            self.aug_stds[self.name2index[name]] = std
                    aug_stds_as_tensor = torch.tensor(self.aug_stds).reshape(1,1,len(self.aug_stds)).repeat(data.shape[0], data.shape[1], 1)
                    ...
                    assert (data[:,:,time_index].diff(n=1, dim=1).nan_to_num(1.)>0).all(), logging.warning(f"Wrong time reordering")
                    logging.debug(f"Reordered times in data: {data.shape=}")
                    
                if self.cfg.do_norm:
                    if self.means is None or self.stds is None:
                        self.means, self.stds = [0.]*data.shape[2], [1.]*data.shape[2]
                    ...
                    means_as_tensor = torch.tensor(self.means).reshape(1,1,len(self.means)).repeat(data.shape[0], data.shape[1], 1)
