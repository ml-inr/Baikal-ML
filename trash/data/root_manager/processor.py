...
try:
    ...
    from data.root_manager.settings import ProcessorConfig, FilterParams
except ImportError:
...
class Processor:
    ...
    def _extract_prefixes(paths: List[str]) -> List[str]:
        """
        Extracts prefixes based on file paths, used to differentiate between file types (mu, nu, etc.).

        Args:
            paths (List[str]): List of paths to root files.

        Returns:
            List[str]: List of extracted prefixes for each file.
        """
        prefixes = []
        for path in paths:
    ...
    def _read_root2df(self) -> Tuple[pl.DataFrame, pl.DataFrame, pl.DataFrame]:
        ...
        Returns:
            List[Tuple[pl.DataFrame, pl.DataFrame, pl.DataFrame]]: DataFrames for events, pulses, and muons.
        """
    ...
    def calc_tres(
        self, muons_df: pl.DataFrame, df_pulses_flat: pl.DataFrame
    ) -> pl.DataFrame:
    ...
    def process_mc_data(
        ...
        pulses_df: pl.DataFrame,
        muons_df: pl.DataFrame,
        OM_coords: pl.DataFrame,
        ...
        df_pulses_flat = df_pulses_flat.with_columns(
            ...
            cluster_id=(pl.col("PulsesChID") // Cnst.CHANNEL_DIVISOR).cast(pl.Int8),
            string_id=(pl.col("PulsesChID") // Cnst.STRING_DIVISOR),
        ...
        # Get cluster centers and relative coords
        cl_centers = (
            OM_coords[["X", "Y", "Z", "cluster_id"]]
            .group_by(["cluster_id"])
            .mean()
            .rename({"X": "Xc", "Y": "Yc", "Z": "Zc"})
        )
        # Join the cluster centers back to the original DataFrame
        OM_coords = OM_coords.join(cl_centers, on="cluster_id").with_columns(
            [
                (pl.col("X") - pl.col("Xc")).cast(pl.Float32).alias("Xrel"),
                (pl.col("Y") - pl.col("Yc")).cast(pl.Float32).alias("Yrel"),
                (pl.col("Z") - pl.col("Zc")).cast(pl.Float32).alias("Zrel"),
            ]
        )
        # Join OM coordinates to pulses DataFrame
        df_pulses_flat = df_pulses_flat.join(
            OM_coords[["ev_id", "PulsesChID", "X", "Y", "Z", "Xrel", "Yrel", "Zrel"]],
            on=["ev_id", "PulsesChID"],
            how="left",
        )
        ...
        final_df = final_df.with_columns(
            nu_induced=pl.col("ev_id").str.starts_with("nu")
        )
        final_df = final_df.with_columns(
            enough_info=(pl.col("num_signal_hits") >= 5)
            & (pl.col("num_signal_strings") >= 2)
        )

        return final_df, {
            "NumEventsAfterFilter": final_df.shape[0],
            "NumEventsBeforeFilter": events_df.shape[0],
        }
    ...
    def process(self) -> pl.DataFrame:
        ...
        events_dfs, pulses_dfs, muons_dfs, OM_coords = self._read_root2df()

        # t0 = time()
        processed_results = Parallel(n_jobs=5)(
            delayed(self.process_mc_data)(df_events, df_pulses, df_muons, OM_coords)
            for df_events, df_pulses, df_muons, OM_coords in zip(
                events_dfs, pulses_dfs, muons_dfs, OM_coords
            )
        )
        # print(f"Time for procesing: {time()-t0}")

        processed_dfs, stats_dicts_to_merge = zip(*processed_results)

        self.filter_koef = sum(
            stats_to_merge["NumEventsAfterFilter"]
            for stats_to_merge in stats_dicts_to_merge
        ) / sum(
            stats_to_merge["NumEventsBeforeFilter"]
            for stats_to_merge in stats_dicts_to_merge
        )
