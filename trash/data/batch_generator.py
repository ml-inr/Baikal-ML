...
class BatchGenerator:
    ...
    def _df_to_tensors_list(self, df: pl.DataFrame, device: torch.device = torch.device('cpu')) -> list[Tensor]:
        """
        ...
        torch_tensors = list(
            map(
                ...
                map(lambda x: np.stack(x, axis=-1), list(df.to_numpy()))
                )
            )
        #torch_tensors = [feature.to(device=device) for feature in torch_tensors]
    ...
    def _process_data_in_batch(self, batch_list_of_tensors: list[Tensor]) -> Tensor:
        features_batch = pad_sequence(batch_list_of_tensors, batch_first=True, padding_value=float('nan'))
    ...
    def _process_labels_in_batch(self, batch_from_chunk: pl.DataFrame, device: torch.device) -> Tensor:
        ...
        if regime == "munusep_2":
            ...
            labels_batch = torch.tensor(batch_from_chunk["onehot_encoding"].to_list(), device=device)
            ...
        else:
            raise ValueError
    ...
    def get_batches(self, device = torch.device("cuda" if torch.cuda.is_available() else "cpu"), yield_small_chunk: bool = False) -> Generator[tuple[Tensor, Tensor, Tensor], None, None]:
        ...
        for chunk in self.chunks:
            ...
            list_of_data_as_tensors = self._df_to_tensors_list(data_in_chunk, device=device) # compute once here for better usage of GPU after.
            logging.info(f"DataFrame converted to list of Tensors.")
            
            # Iterate to get batches
            for start in range(0, chunk.shape[0], self.batch_size):
                stop = start + self.batch_size
                # pad data in batch with nans
                features_batch = self._process_data_in_batch(list_of_data_as_tensors[start:stop]) # uses device same with any tensor in list
                # get batch of labels
