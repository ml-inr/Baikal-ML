"""Shared inference utilities for all Baikal-GVD model notebooks and scripts.

Imported by inference/prefilter_model/utils.py and
inference/nu_classifier_model/utils.py — do not import those from here.
"""
...
# mc_reco.h5 — 25 scalars (same as EXP_RECO_COL_NAMES) + 6 MC-specific vectors
...
def load_mc_test_parts(
    ...
    Args:
        ...
        train_parts_json: Path to training parts JSON.
        n_parts_per_particle: e.g. {"muatm": 50, "nue2": 10, "nuatm": 10}.
        seed: Random seed for part selection.

    Returns:
        DataFrame with columns: particle_type, n_hits, n_signal_hits,
        n_signal_strings, features, signal_mask,
        [theta, phi, energy if prime_prty present],
        [mu_ind_* columns if muons_prty present].
    """
    ...
    with h5py.File(h5_path, "r") as h5:
        for ptype_short, n_parts in n_parts_per_particle.items():
            ...
            for part_num in tqdm(selected, desc=f"MC {ptype_short}", unit="part"):
                ...
                muons_prty  = grp[f"muons_prty/individ/{part_name}/data"][:] if has_mu_prty else None
                muon_starts = grp[f"muons_prty/mu_starts/{part_name}/data"][:] if has_mu_prty else None
                ...
                for i in range(n_events):
                    ...
                    if muons_prty is not None:
                        ms, me = int(muon_starts[i]), int(muon_starts[i + 1])
                        row["mu_ind_theta"]  = muons_prty[ms:me, 0]
                        row["mu_ind_phi"]    = muons_prty[ms:me, 1]
                        row["mu_ind_x"]      = muons_prty[ms:me, 2]
                        row["mu_ind_y"]      = muons_prty[ms:me, 3]
                        row["mu_ind_z"]      = muons_prty[ms:me, 4]
                        row["mu_ind_t"]      = muons_prty[ms:me, 5]
                        row["mu_ind_energy"] = muons_prty[ms:me, 6]
                    rows.append(row)
...
def load_exp_parts(
    ...
    Args:
        ...
        max_parts: Cap number of HDF5 parts to read (None = all).
        seed: Seed for subsampling if max_events is set.
        load_reco: If True, also read reco_prty and reco_hit_prty columns.

    Returns:
        DataFrame with columns: h5_part_str, local_id, n_hits, max_q, mean_z,
        features, [signal_mask], [reco columns], [header columns].
    """
    ...
    col_reco_hit:           List[np.ndarray] = []
    col_n_signal_hits_reco: List[int]        = []
...
def load_mc_reco_parts(
    ...
    Returns DataFrame with particle_type, h5_part_str, local_id, n_hits,
    features, signal_mask, MC truth columns, and named reco columns
    (MC_RECO_COL_NAMES).
    """
...
def predict_scores(
    ...
    Args:
        ...
        feats_with_probs: If True, append sig-noise prob as 6th feature and
            extend normalization with (mean=0.75, std=0.25).
...
def plot_event_3d(
    ...
    """Plot a single event in 3D (amplitude, time, x, y, z)."""
    ...
    if noise_mask.any():
        ax.scatter(x[noise_mask], y[noise_mask], z[noise_mask],
                   s=sizes[noise_mask], c="grey", alpha=0.25, linewidths=0,
                   label=f"Noise ({noise_mask.sum()})")

    if signal_mask.any():
        t_sig  = time[signal_mask]
        t_norm = (t_sig - t_sig.min()) / (t_sig.max() - t_sig.min() + 1e-8)
        sc = ax.scatter(x[signal_mask], y[signal_mask], z[signal_mask],
                        s=sizes[signal_mask], c=t_norm, cmap="coolwarm_r",
...
def load_blind_reco(
    ...
    Renames fSeason/fCluster/fRun/fEvent to match load_exp_parts column names.
    """
    try:
        import uproot as ur
    ...
    files = [f for f in all_files if ("prelCuts" in f.name) == prelcuts_only]
    if not files:
    ...
    logger.info(f"Loaded {len(df):,} blind reco events from {len(files)} files")
    return df


# ═══════════════════════════════════════════════════════════════════════
#  Nu-classifier NPY dataset loading
# ═══════════════════════════════════════════════════════════════════════
...
def load_npy_exp(
    ...
    """Load exp events from nu_classifier NPY dataset into a DataFrame."""
    ...
    if feats_with_probs:
        ...
        feats_col = [np.array(features_mmap[offsets[i]:offsets[i + 1]]) for i in idx]
...
def load_raw_hits_for_npy_exp(
    ...
    Expects df["part_names"] entries formatted as "{part_key}_{event_idx}".
    Adds columns: raw_channels, raw_times, raw_charges.
    """
    ...
    part_to_rows: Dict[str, List] = defaultdict(list)
    for row_pos, (part_key, event_idx) in enumerate(parsed):
        part_to_rows[part_key].append((row_pos, event_idx))

    raw_channels: List = [None] * len(df)
    raw_times:    List = [None] * len(df)
    raw_charges:  List = [None] * len(df)

    with h5py.File(h5_path, "r") as h5:
        exp_grp = h5["exp_reco"] if "exp_reco" in h5 else h5["exp"]
        for part_key, row_events in part_to_rows.items():
            ev_starts = exp_grp[f"raw/ev_starts/{part_key}/data"][:].astype(np.int64)
            data_raw  = exp_grp[f"raw/data/{part_key}/data"][:]
            channels  = exp_grp[f"raw/channels/{part_key}/data"][:]
