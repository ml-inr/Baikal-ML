"""Supervised fine-tuning trainer — legacy augmentation (pre-fix copy).

Kept to reproduce results trained before the augmentation fix.
Differences from exp_finetuning_trainer.py:
  - noise_std values are applied directly to already-normalized features
    (no division by normalization_config["stds"])
  - collate-level augmentation is enabled in both MC and BG loaders in
    addition to the _augment() call in train_epoch (double augmentation)
