...
def load_mc_test_parts(
    ...
    with h5py.File(h5_path, "r") as h5:
        for ptype_short, n_parts in n_parts_per_particle.items():
            ...
            has_mu_party = "muons_prty" in grp
            ...
            for part_num in tqdm(selected, desc=f"MC {ptype_short}", unit="part"):
                ...
                # individual muons info
                muons_prty = grp[f"muons_prty/individ/{part_name}/data"][:] if has_mu_party else None
                muon_starts = grp[f"muons_prty/mu_starts/{part_name}/data"][:] if has_mu_party else None
                ...
                for i in range(n_events):
                    ...
                    if muons_prty is not None:
                        row['mu_ind_theta'] = muons_prty[muon_starts[i]:muon_starts[i+1], 0]
                        row['mu_ind_phi'] = muons_prty[muon_starts[i]:muon_starts[i+1], 1]
                        row['mu_ind_x'] = muons_prty[muon_starts[i]:muon_starts[i+1], 2]
                        row['mu_ind_y'] = muons_prty[muon_starts[i]:muon_starts[i+1], 3]
                        row['mu_ind_z'] = muons_prty[muon_starts[i]:muon_starts[i+1], 4]
                        row['mu_ind_t'] = muons_prty[muon_starts[i]:muon_starts[i+1], 5]
                        row['mu_ind_energy'] = muons_prty[muon_starts[i]:muon_starts[i+1], 6]
                    rows.append(row)
...
def predict_scores(
    ...
    for start in tqdm(range(0, n, batch_size), desc="Predicting", unit="batch", total=(n + batch_size - 1) // batch_size):
